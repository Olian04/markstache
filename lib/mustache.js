    ...
    Object.keys(fragmentManifest).forEach(k => {
        if (fragmentManifest[k].uri) {
            if (!fs.exists(path.join(root, fragmentManifest[k].uri))) {
                console.log(`[${Color.RED}FAIL${Color.RESET}][${Color.PURPLE}URI${Color.RESET}]: ${k}`)
            } else {
            ...
            requests.push(
                fetch(fragmentManifest[k].url)
                    ...
                    .catch( e => { console.log(`[${Color.RED}FAIL${Color.RESET}][${Color.BLUE}URL${Color.RESET}]: ${k} (${e.code})`); progressBar.tick(); })
            );
        }
    });
