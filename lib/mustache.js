...
module.exports = async function mustache(target, hash) {
    ...
    const progressBar = new ProgressBar(`${Color.GREEN}:ok${Color.RESET}|${Color.RED}:fail${Color.RESET} [:bar] :etas`, {
        complete: '=',
        incomplete: ' ',
        width: 20,
    ...
    Object.keys(fragmentManifest).forEach(k => {
        if (fragmentManifest[k].uri) {
            if (!fs.exists(path.join(root, fragmentManifest[k].uri))) {
                tickState.fail++;
                logs.push(`[${Color.RED}FAIL${Color.RESET}][${Color.PURPLE}URI${Color.RESET}]: ${k} ${Color.WHITE}FILE NOT FOUND${Color.RESET}`)
            } else {
                tickState.ok++;
                fragments[k] = fs.read(path.join(root, fragmentManifest[k].uri));
                logs.push(`[${Color.GREEN}OK${Color.RESET}][${Color.PURPLE}URI${Color.RESET}]: ${k}`);
            }
            progressBar.tick(tickState);
            ...
            requests.push(
                fetch(fragmentManifest[k].url)
                    ...
                    .then( () => { 
                        tickState.ok++;
                        logs.push(`[${Color.GREEN}OK${Color.RESET}][${Color.BLUE}URL${Color.RESET}]: ${k}`); 
                        progressBar.tick(tickState); 
                    })
                    .catch( e => { 
                        tickState.fail++;
                        logs.push(`[${Color.RED}FAIL${Color.RESET}][${Color.BLUE}URL${Color.RESET}]: ${k} ${Color.WHITE}${e.code}${Color.RESET}`); 
                        progressBar.tick(tickState); 
                    })
            );
        }
