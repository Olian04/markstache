        ...
        if (fragmentManifest[k].uri) {
            ...
            requests.push(
                fetch(fragmentManifest[k].url)
                    ...
                    .then( () => { 
                        tickState.ok++;
                        console.log(`[${Color.GREEN}OK${Color.RESET}][${Color.BLUE}URL${Color.RESET}]: ${k}`); 
                        progressBar.tick(tickState); 
                    })
                    .catch( e => { 
                        tickState.fail++;
                        console.log(`[${Color.RED}FAIL${Color.RESET}][${Color.BLUE}URL${Color.RESET}]: ${k} ${Color.WHITE}${e.code}${Color.RESET}`); 
                        progressBar.tick(tickState); 
                    })
            );
        }
